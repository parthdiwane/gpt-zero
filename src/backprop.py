...
class Tensor:
    ...
    def apply(name, *args, **kwargs):
        ...
        if any(needs_grad_tensors):
            return Tensor(data=out, requires_grad=True, _node=Node(operation=name, parents=parents, cache=cache,needs=needs_grad_tensors))
        else:
...
SCHEMA = { # {opeartion : {"saved_inputs: ":, "saves_results: bool":}}
    ...
    "softmax": {"params": ("x", "axis"),
                "defaults": {"axis": -1},
                "differentiable": (0,)},

    "sum":     {"params": ("x", "axis", "keepdims"),
                "defaults": {"axis": None, "keepdims": False},
                "differentiable": (0,)},

    "layernorm": {"params": ("x", "gamma", "beta", "axis", "eps"),
                  "defaults": {"axis": -1, "eps": 1e-5},
                  "differentiable": (0, 1, 2)},

    "embedding": {"params": ("weight", "ids"),
                  "defaults": {},
                  "differentiable": (0,)},
}
